...
export default async function handler(
    ...
    if (req.method === GET) {
        ...
        const group = await db.collection('availabilities').find({ instructorId: new ObjectId(instructorId) }).toArray();

        if (!group) {
            return res.status(404).json({
                message: "Not Found",
                error: `No availability found for instructor with ID ${instructorId}`,
                status: 404
            });
        }
