...
import { GET, PATCH, POST } from "../../../../lib/constants/http.methods.constant";
    ...
    if (req.method === PATCH) {
        ...
        return completeScheduleSchema.validate({ note })
                ...
                try {
                    ...
                    if (schedule.status !== COMPLETED) {
                        return res.status(400).send({
                            message: `Bad Request`,
                            status: 400,
                            error: `You can only update note for a completed schedule`,
                        })
                    }

                    /* 
                    *   TODO
                    *   ALLOW ONLY THE OWNER OF THE SCHEDULE TO UPDATE NOTE
                     */
            ...
            .catch(error => {
                ...
                res.status(400).send({
                    message: "Bad request",
                    error: "Possible note is missing",
                    status: 400
                });
            })
